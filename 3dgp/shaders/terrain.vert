...
void main(void) 
{
	...
	float povAlt = dot(-position.xyz, mat3(matrixModelView) * vec3(0, 1, 0));

	// calculate fog
	float fogDist = length(position.xyz);
	float clampWD = max(waterDepth, 0);
	float clampPOV = max(povAlt, 1);
	float submerged = clampWD / clampPOV;

	fogFactor = exp2(-fogDensity * fogDist * submerged);
